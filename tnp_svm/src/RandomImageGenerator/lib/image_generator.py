...
class ImageGenerator():
    def __init__(self, item_path, background_path):
        ...
        self.item_files = glob.glob(item_path + "/*.png")
        self.items = []
        self.labels = []
        self.bgs = []
    ...
    def generate_samples(self, n_samples, n_items, crop_width, crop_height, min_item_scale, max_item_scale, rand_angle, minimum_crop, delta_hue, delta_sat_scale, delta_val_scale):
        ...
        for i in range(n_samples):
            ...
            for j in range(np.random.randint(n_items)+1):
                ...
                item = self.items[class_id]
                OrigItem2CropBackgroundWidthRate = float(item.shape[0]/crop_width)
                OrigItem2CropBackgroundHeightRate = float(item.shape[1]/crop_height)
                if OrigItem2CropBackgroundWidthRate > 1.0 or OrigItem2CropBackgroundHeightRate > 1.0:
                    resize_rate = min(OrigItem2CropBackgroundWidthRate, OrigItem2CropBackgroundHeightRate)
                    item = scale_image(item,1/resize_rate)
