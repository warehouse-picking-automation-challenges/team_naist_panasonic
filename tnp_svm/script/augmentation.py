...
def paste_on_black(image_in):
	...
	if width >= height:
		...
		background[center_w-center_h:center_w+center_h,0:center_w*2] = image_in[0:int(height/2)*2,0:int(width/2)*2]
	else:
		background = cv2.imread("/root/catkin_ws/src/tnp/tnp_svm/script/background/blackimage.png")
		background = cv2.resize(background, (height,height))
		background[0:center_h*2,center_h-center_w:center_h+center_w] = image_in[0:int(height/2)*2,0:int(width/2)*2]
	return background
