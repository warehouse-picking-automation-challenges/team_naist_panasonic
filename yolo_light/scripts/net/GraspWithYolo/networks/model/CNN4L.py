...
class CNNL(BaseModel):
    ...
    def __init__(self, name, in_channels,seed=1):
        ...
        with tf.variable_scope(self.name):
            ...
            self.l7 = Linear('l6',512,512,nobias=False)
            self.bn7 = BatchNormalization('bn6',512,dim=1)
            self.l8 = Linear('l7',512,4,nobias=False)
    ...
    def forward_tanh(self,x,batchsize,train):
        ...
        output = tf.nn.tanh(self.bn7(self.l7(output),train=train))
        output = tf.nn.tanh(self.l8(output))
        return output
