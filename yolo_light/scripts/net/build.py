...
class TFNet(object):
	...
	def __init__(self, FLAGS, darknet = None):
		...
		if isinstance(FLAGS, dict):
			defaultSettings = {"binary": "./bin/", "config": "./cfg/", "batch": 16, "threshold": 0.1,"threshold_max": 1.0, "train": False, "verbalise": False, "gpu": 0.0,"log":"","use_gpu_num" : "", "show_device_info":False, "test_output": "" ,"pred_grasp":False }
	...
	def setup_meta_ops(self):
		cfg = dict({
			...
			'log_device_placement': self.FLAGS.show_device_info
		})
		...
		if utility > 0.0:
			...
			if self.FLAGS.use_gpu_num != "":
				self.say('Running on pointed GPU')
				cfg['gpu_options'] = tf.GPUOptions()
				cfg['allow_soft_placement'] = True
			else:
	...
	def setup_log(self):
		with open(self.FLAGS.log,"a") as f:
			f.write("Start on - "+time.ctime()+"\n")
