...
class Camera:
	...
	def record_callback(self,CartesianPose):
		...
		#z=0.50m
		if  0.495<height and height<0.504:
			self.high_Trigger = True
			self.middle_Trigger = False
			self.low_Trigger = False
		#z=0.45m
		if 0.445<height and height<0.454:
			self.high_Trigger = False
			self.middle_Trigger = True
			self.low_Trigger = False
		#z=0.40m
		if 0.395<height and height<0.404:
			self.high_Trigger = False
			self.middle_Trigger = False
	...
	def main(self):
		...
		###
		led_intensity=[]
		led_intensity=["led150","led100","led050"]
		led_str=[]
		led_str=["00F150","00F100","00F050"]
		SavePath_rgb_high=[]
		SavePath_rgb_high=[SavePath_rgb_led050_high,SavePath_rgb_led150_high,SavePath_rgb_led100_high]
		SavePath_rgb_middle=[]
		SavePath_rgb_middle=[SavePath_rgb_led050_middle,SavePath_rgb_led150_middle,SavePath_rgb_led100_middle]
		SavePath_rgb_low=[]
		SavePath_rgb_low=[SavePath_rgb_led050_low,SavePath_rgb_led150_low,SavePath_rgb_led100_low]
		...
		while not rospy.is_shutdown():
			...
			if self.high_Trigger == True:
				...
				if self.CaptureTrigger_depth == True:
					Filename_depth = Savename + "_" +'{0:0>3}'.format(self.CombinationCount_high)+"_high"+"_depth"+".png"
					cv2.imwrite(SavePath_depth_image_high+"/"+Filename_depth,self.depth_image_cv2)
					print('depth_image')
				if self.CaptureTrigger_depth_registered == True:
					Filename_depth_registered = Savename+"_"+'{0:0>3}'.format(self.CombinationCount_high)+"_high"+"_depth_registered"+".png"
				...
				if self.CaptureTrigger_depth_data == True:
					Filename_depth_data = Savename + "_" +'{0:0>3}'.format(self.CombinationCount_high)+"_high"+"_depth_data_"+".dat"
			...
			if self.middle_Trigger == True:
				...
				if self.CaptureTrigger_rgb == True:
					...
					for i in range(3):
						...
						print(led_intensity[i])
				if self.CaptureTrigger_depth == True:
					Filename_depth = Savename + "_" +'{0:0>3}'.format(self.CombinationCount_middle)+"_middle"+"_depth"+".png"
					cv2.imwrite(SavePath_depth_image_middle+"/"+Filename_depth,self.depth_image_cv2)
					print('depth_image')
				if self.CaptureTrigger_depth_registered == True:
					Filename_depth_registered = Savename+"_"+'{0:0>3}'.format(self.CombinationCount_middle)+"_middle"+"_depth_registered"+".png"
					cv2.imwrite(SavePath_depth_image_registered_middle+"/"+Filename_depth_registered,self.depth_image_registered_cv2)
					print('depth_registered')
				if self.CaptureTrigger_depth_data == True:
					Filename_depth_data = Savename + "_" +'{0:0>3}'.format(self.CombinationCount_middle)+"_middle"+"_depth_data_"+".dat"
			...
			if self.low_Trigger == True:
				...
				if self.CaptureTrigger_rgb == True:
					...
					for i in range(3):
						...
						print(led_intensity[i])
				if self.CaptureTrigger_depth == True:
					Filename_depth = Savename + "_" +'{0:0>3}'.format(self.CombinationCount_low)+"_low"+"_depth"+".png"
					cv2.imwrite(SavePath_depth_image_low+"/"+Filename_depth,self.depth_image_cv2)
					print('depth_image')
				if self.CaptureTrigger_depth_registered == True:
					Filename_depth_registered = Savename+"_"+'{0:0>3}'.format(self.CombinationCount_low)+"_low"+"_depth_registered"+".png"
					cv2.imwrite(SavePath_depth_image_registered_low+"/"+Filename_depth_registered,self.depth_image_registered_cv2)
					print('depth_registered')
				if self.CaptureTrigger_depth_data == True:
					Filename_depth_data = Savename + "_" +'{0:0>3}'.format(self.CombinationCount_low)+"_low"+"_depth_data_"+".dat"
