...
void drawFilledCylinder(cv::Mat result, cv::Mat KRGB, cv::Mat pose, cv::Point3f baseCenter, cv::Point3f top, float radius, cv::Scalar color, int nbHeightSegment = 10, int nbRadiusSegment = 20);
...
void createArucoData(float squareLength, float markerLength, cv::Ptr<cv::aruco::Dictionary>& dictionary, cv::Ptr<cv::aruco::CharucoBoard>& boardTop, cv::Ptr<cv::aruco::CharucoBoard>& boardSmallSide1,
                     cv::Ptr<cv::aruco::CharucoBoard>& boardSmallSide2, cv::Ptr<cv::aruco::CharucoBoard>& boardLongSide1, cv::Ptr<cv::aruco::CharucoBoard>& boardLongSide2, cv::Ptr<cv::aruco::CharucoBoard>& additionalBoard, bool saveToDisk = false);
