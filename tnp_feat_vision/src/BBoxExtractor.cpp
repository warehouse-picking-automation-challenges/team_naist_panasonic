...
cv::Mat BBoxExtractor::generateView(const RgbdFrame& frame, const std::vector<cv::Point3f>& recoBBOX, const std::vector<cv::Point3f>& OBB, const std::vector<cv::Point3f>& AABB)
{
    int id = getIdBySerial(frame.camSerial);
    cv::Mat KRGB = cameraK[id];//frame.getKMat(frame.color_intrin);
...
void BBoxExtractor::compute(std::vector<RgbdFrame> listFrames)
{
    ...
    realCylinderPosX = suctionQRFound?(qrHeight-0.035+suctionCylinderHeight/2):suctionCylinderCenter.x;
    //while(true)
    {
        ...
        if(drawResult)
        {
            for(int i = 0; i < listFrames.size(); i++)
            {
                cv::Mat result = generateView(listFrames[i], recoBBOX, OBB, AABB);
                char name[255];
